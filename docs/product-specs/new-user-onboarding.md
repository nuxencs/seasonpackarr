...
4. Configure one or more torrent clients with `preImportPath` and the matching `type`:
   - `qbittorrent` (default): username/password credentials, a qBittorrent 5.2.0+ `apiKey`, or a qui reverse-proxy host with client auth left blank.
   - `transmission`: username/password credentials (no `apiKey`); the RPC protocol version is auto-negotiated.
