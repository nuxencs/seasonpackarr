...
| Prowlarr discovery | 3 | API/CLI fixtures cover grouping, tracker selection, search-only preview, source preflight, metadata reuse, exact imports, duplicates, pagination, rate limits, cancellation, RSS catch-up, retained candidates, client recovery, SQLite restart persistence, and transactional rollback | Live Prowlarr/tracker validation; ambiguous and date-based episode names |
